        ...
        @if ($this->isManager())
            <section class="assessment-extracurricular-workspace" aria-label="Pengelolaan peserta ekskul">
                ...
                <div class="assessment-extracurricular-section-grid">
                    <div class="assessment-extracurricular-section"><div class="assessment-extracurricular-section__heading"><div><h2>Tambah peserta manual</h2><p>Pilih ekskul dan kelas, lalu tambahkan beberapa siswa sekaligus.</p></div></div><div class="assessment-extracurricular-form"><div class="assessment-extracurricular-field"><label for="participant-extracurricular">Ekskul</label><select id="participant-extracurricular" wire:model="selectedExtracurricularId" class="fi-input"><option value="">Pilih ekskul</option>@foreach($this->extracurriculars as $activity)<option value="{{ $activity->id }}">{{ $activity->name }}</option>@endforeach</select></div><div class="assessment-extracurricular-field"><label for="participant-rombel">Kelas</label><select id="participant-rombel" wire:model.live="selectedRombelId" class="fi-input"><option value="">Pilih kelas</option>@foreach($this->rombelOptions as $id => $name)<option value="{{ $id }}">{{ $name }}</option>@endforeach</select></div></div>@if($selectedRombelId)<div class="assessment-extracurricular-class-picker"><div class="assessment-extracurricular-class-picker__toolbar"><strong>{{ $this->classStudentOptions->count() }} siswa di kelas ini</strong><span>{{ count($selectedStudentIds) }} dipilih</span><button type="button" wire:click="selectAllClassStudents">Pilih semua siswa kelas ini</button><button type="button" wire:click="$set('selectedStudentIds', [])">Bersihkan pilihan</button></div><div class="assessment-extracurricular-student-grid">@forelse($this->classStudentOptions as $student)<label><input type="checkbox" value="{{ $student->id }}" wire:model.live="selectedStudentIds"><span>{{ $student->student_name_snapshot }}</span><small>{{ $student->nis_snapshot ?: 'NIS belum tersedia' }}</small></label>@empty<p class="assessment-extracurricular-empty">Belum ada siswa aktif di kelas ini. Pilih kelas lain atau periksa data siswa periode ASTS.</p>@endforelse</div><div class="assessment-extracurricular-button"><x-filament::button color="gray" wire:click="addSelectedParticipants" :disabled="!$this->classStudentOptions->count() || !count($selectedStudentIds)">Tambah peserta terpilih</x-filament::button></div></div>@endif<div class="assessment-extracurricular-form"><div class="assessment-extracurricular-field"><label for="participant-student">Tambah satu siswa (opsional)</label><select id="participant-student" wire:model="selectedStudentId" class="fi-input"><option value="">Pilih siswa</option>@foreach($this->studentOptions as $id => $name)<option value="{{ $id }}">{{ $name }}</option>@endforeach</select></div><div class="assessment-extracurricular-button"><x-filament::button color="gray" wire:click="addParticipant">Tambah satu peserta</x-filament::button></div></div></div>
